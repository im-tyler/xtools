...
AI Post Studio stores its accounts, drafts, voice material, queue, and API key in browser extension storage. It can generate through DeepSeek, OpenAI, or a custom OpenAI-compatible endpoint. Scheduled posts run only while X is available in the browser and are bounded by the configured rate limit and active-hours window.
